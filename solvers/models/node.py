...
class Node:
    ...
    def __init__(self, instance: PathSelectionProblem, used_routes: set[int], timelimit: int, description: str = "",
        ...
        # TODO use env and model that are shared among the nodes
        env = gp.Env(empty=True)
        env.setParam('OutputFlag', 0)
        env.start()
        self.model = gp.Model("RMP", env=env)
        # sometime the timelimit is negative because of the time taken by the previous operation, in this case we set it to 0
    ...
    def solve_pricing(self, dual_values, columns_number=10):
        ...
        for i in range(len(prices)):
            ...
            if prices[i][0] not in self.used_routes and prices[i][0] not in forbidden_routes:
                ...
                self.logger.debug(
                    "Add route {} ({}), reduce cost = {}".format(self.instance.get_route(prices[i][0]), prices[i][0],
                                                                 prices[i][1]))
    ...
    def solve(self, columns_number=10):
        ...
        """
        self.iterations = 0
        self.logger.debug("Initial RMP with {} columns".format(len(self.used_routes)))
        ...
        self.logger.debug("Objective : {}".format(self.objective_value))

        new_routes = self.solve_pricing(dual_values, columns_number)
        self.iterations = 1
        while len(new_routes) > 0 and self.timelimit > 0:
            self.logger.debug("Adding {} new routes".format(len(new_routes)))
            ...
            if len(new_routes) == 0:
                self.logger.debug("End of column generation with {}% of columns ({})".format(
                    round(float(len(self.used_routes)) / self.instance.route_number * 100, 2),
                    len(self.used_routes)))
                self.finished = True
            self.iterations += 1
